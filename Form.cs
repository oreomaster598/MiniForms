...
namespace MiniForms
{
    ...
    {
        internal static Form runing;
        internal static bool isformruning ;
    ...
    public static class Icons
    {
        ...
        public static Icon LoadApplicationIcon(IntPtr Handle)
        {
            return new Icon(LoadIcon(Handle, (IntPtr)SystemIcons.IDI_APPLICATION));
        }//IDI_ERROR
        public static Icon LoadErrorIcon(IntPtr Handle)
        {
            return new Icon(LoadIcon(Handle, (IntPtr)SystemIcons.IDI_ERROR));
        }
    }
    public struct HWND
    {
        ...
        public static implicit operator IntPtr(HWND ptr) => ptr.hwnd;
    }
    public class Form
    {
        #region Consts
   ...
   uint wMsgFilterMax);
        ...
        public void Invalidate(bool clear = false)
        {
            InvalidateRect(hwnd, IntPtr.Zero, clear);
        }
        ...
        internal bool Create()
        {
            ...
            wind_class.cbSize = (uint)Marshal.SizeOf(typeof(WNDCLASSEX));
            wind_class.style = (ClassStyles.HorizontalRedraw | ClassStyles.VerticalRedraw | ClassStyles.DoubleClicks) ;
            wind_class.hbrBackground = GetStockObject(StockObjects.WHITE_BRUSH); 
            wind_class.cbClsExtra = 0;
            wind_class.cbWndExtra = 0;
            wind_class.hInstance = Handle;
            wind_class.hIcon = icon.Handle;//LoadIcon(Handle,  (IntPtr)SystemIcons.IDI_APPLICATION);
            wind_class.hCursor = LoadCursor(Handle, (int)IDC_STANDARD_CURSORS.IDC_ARROW);
            wind_class.lpszMenuName = null;
            wind_class.lpszClassName = "myClass";
            wind_class.lpfnWndProc = wndProc;

            uint wstyles = WindowStyles.WS_VISIBLE | WindowStyles.WS_BORDER | WindowStyles.WS_OVERLAPPEDWINDOW;
            uint wstylesex = (uint)( WindowStyles.WS_EX_NOPARENTNOTIFY);
            ...
            hwnd = CreateWindowEx(wstylesex, regResult, Title, wstyles  , Window_Rect.X, Window_Rect.Y, Window_Rect.Width, Window_Rect.Height, IntPtr.Zero, IntPtr.Zero, wind_class.hInstance, IntPtr.Zero);;
        ...
        public void Update()
        {
            IntPtr hWnd = FindWindow("myClass", Title);
            UpdateWindow(hWnd);
        }

        public virtual void OnPaint(IntPtr graphicsDevice, RECT client)
        {
        ...
        private static unsafe IntPtr WindowProcess(IntPtr hWnd, uint msg, IntPtr wParam, IntPtr lParam)
        {
            IntPtr hdc;
            PAINTSTRUCT ps;
            RECT rect;
            ...
            {               
                ...
                case WM.PAINT:
                    ...
                    if (Application.isformruning)
                        Application.runing.OnPaint(hdc,rect);
                ...
                case WM.KEYDOWN:
                    if (Application.isformruning)
                        Application.runing.OnKeyDown();
                    break;
                case WM.LBUTTONDOWN:
                    if (Application.isformruning)
                        Application.runing.OnMouseDown(MouseButton.Left);
                    break;
                case WM.RBUTTONDOWN:
                    if (Application.isformruning)
                        Application.runing.OnMouseDown(MouseButton.Right);
                    break;
                case WM.MBUTTONDOWN:
                    if (Application.isformruning)
                        Application.runing.OnMouseDown(MouseButton.Middle);
                    break;
                case WM.MOVING:
                    if (Application.isformruning)
                        Application.runing.OnMove();
                    break;
                case WM.DESTROY:
                    if(Application.isformruning)
                        Application.runing.OnDestroy();
                    Application.isformruning = false;
                    PostQuitMessage(0);
                    break;
